...
def load_model_json():
    ...
    # no_sdaa: Disables SDAA training (**To be implemented)
    # seed: Random seed
...
def main(args):
    ...
    for i, data in enumerate(dataloader):
        ...
        for n in range(res.shape[0]): # loop over each image
            ...
            if sdaa_available:
              input_image = input_image.sdaa()
              lbl_image = lbl_image.sdaa()
              res_image = res_image.sdaa()

            compare_image = torch.cat((input_image, lbl_image, res_image), dim=2)

            if sdaa_available:
              compare_image = compare_image.sdaa()
