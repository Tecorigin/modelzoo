...
def train_pnet(model_store_path, end_epoch,imdb,
    ...
    for cur_epoch in range(1,end_epoch+1):
        ...
        accuracy_avg = torch.mean(torch.cat([acc.unsqueeze(0) for acc in accuracy_list]))
        cls_loss_avg = torch.mean(torch.cat([loss.unsqueeze(0) for loss in cls_loss_list]))
        bbox_loss_avg = torch.mean(torch.cat([loss.unsqueeze(0) for loss in bbox_loss_list]))
...
def train_onet(model_store_path, end_epoch,imdb,
    ...
    for cur_epoch in range(1,end_epoch+1):
        ...
        cls_loss_avg = torch.mean(torch.cat([loss.unsqueeze(0) for loss in cls_loss_list]))
        bbox_loss_avg = torch.mean(torch.cat([loss.unsqueeze(0) for loss in bbox_loss_list]))
        landmark_loss_avg = torch.mean(torch.cat([loss.unsqueeze(0) for loss in landmark_loss_list]))
