...
class GAN_model(BaseModel):
    ...
    def get_current_losses(self):
        ...
        losses = {}
        # 添加重建损失
        for i, rec_loss in enumerate(self.rec_losses):
            losses[f'rec_loss_{i}'] = rec_loss.item()
        
        # 添加其他损失
        losses['cycle_loss'] = self.cycle_loss.item()
        losses['ee_loss'] = self.ee_loss.item()
        losses['D_loss'] = self.loss_D.item()
        losses['G_loss'] = self.loss_G.item()
        losses['total_loss'] = self.loss_G_total.item()
