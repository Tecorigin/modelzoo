...
def local_dataset(dataset_name):
    # 1) 直接读取 save_to_disk 的目录
    if os.path.isdir(dataset_name):
        try:
            return load_from_disk(dataset_name)
        except Exception as e:
            raise ValueError(f'Arrow dataset dir but failed to load: {e}')

    # 2) 单文件格式
    if dataset_name.endswith(('.json', '.jsonl')):
        full_dataset = Dataset.from_json(dataset_name)            # JSONL / JSON list
