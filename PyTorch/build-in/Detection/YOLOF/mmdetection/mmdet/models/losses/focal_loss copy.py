...
def sigmoid_focal_loss(pred,
    ...
    # === 修改开始: 强制使用 CPU 计算规避 SDAA 算子缺失问题 ===
    # 1. 保存当前设备
    device_orig = pred.device
    
    # 2. 搬运到 CPU
    pred_cpu = pred.detach().cpu().contiguous()
    target_cpu = target.detach().cpu().contiguous()
    
    # 3. 在 CPU 上计算 (MMCV 会自动调用 CPU 实现)
    loss_cpu = _sigmoid_focal_loss(pred_cpu, target_cpu, gamma,
                                   alpha, None, 'none')
    
    # 4. 搬回原始设备
    loss = loss_cpu.to(device_orig)
    # === 修改结束 ===
