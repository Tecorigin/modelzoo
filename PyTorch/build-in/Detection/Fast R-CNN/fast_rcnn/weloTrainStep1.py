...
def generate_proposals(image_path):
    ...
    image_path: 图片的路径
    返回：候选框的列表，每个候选框是 [x, y, x+w, y+h] 格式
    """
...
def build_auto_config(config_rel_path, args, final_work_dir):
    ...
    def get_dataloader_cfg(shuffle=False, pipeline=None):
        # 1. 动态构建 proposals 文件的绝对路径
        # 假设 args.datapath 是 ../data/cat_dog/catdog_coco
        # 文件应该在 ../data/cat_dog/catdog_coco/proposals/proposals_train2017.pkl
        # proposal_file_path = os.path.join(args.datapath, 'proposals/proposals_train2017.pkl')
        ...
        if os.path.exists(proposal_file_path):
            ...
            # 如果是 Fast R-CNN 且找不到文件，必须报错，否则后面会 KeyError
            if 'fast_rcnn' in args.model or 'fast-rcnn' in args.model:
                print(f"[Dataset] ❌ 严重错误: Fast R-CNN 需要 proposal 文件但未找到: {proposal_file_path}")
                # 这里不退出，让它报错以便你看到堆栈，或者你可以选择 sys.exit(1)
            else:
                print(f"[Dataset] ⚠️ 未找到 Proposal 文件 (非 Fast R-CNN 可忽略): {proposal_file_path}")
