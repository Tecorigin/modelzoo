    ...
    while epoch < args.epochs:
        ...
        if semi_supervised:
            ...
            for (_, batch_3d, batch_2d), (cam_semi, _, batch_2d_semi) in \
                ...
                print('Epoch: {} Step: {} 3D Loss: {:.4f} Traj Loss: {:.4f} Recon Loss: {:.4f} Total Loss: {:.4f}'.format(
                    epoch + 1, N // (inputs_3d.shape[0]*inputs_3d.shape[1]), 
                    loss_3d_pos.item() * 1000, 
                    loss_traj.item() * 1000, 
                    loss_reconstruction.item() if not skip and not args.no_proj else 0,
                    loss_total.item()))
            ...
            for _, batch_3d, batch_2d in train_generator.next_epoch():
                ...
                print('Epoch: {} Step: {} 3D Loss: {:.4f} Total Loss: {:.4f}'.format(
                    epoch + 1, N // (inputs_3d.shape[0]*inputs_3d.shape[1]), 
                    loss_3d_pos.item() * 1000, 
                    loss_total.item()))
