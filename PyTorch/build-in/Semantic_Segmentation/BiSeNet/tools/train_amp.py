...
import argparse
import numpy as np
# from tabulate import tabulate  # 移除不需要的导入
...
from configs import set_cfg_from_file
from lib.data import get_data_loader
# from evaluate import eval_model  # 移除不需要的导入
from lib.ohem_ce_loss import OhemCELoss
from lib.lr_scheduler import WarmupPolyLrScheduler
from lib.meters import TimeMeter, AvgMeter
from lib.logger import setup_logger, log_msg
...
def train():
    ...
    for it in range(cfg.max_iter):
        ...
        ## print training log message for every iteration
        lr = lr_schdr.get_lr()
        lr = sum(lr) / len(lr)
        msg = log_msg(
            it, cfg.max_iter, lr, time_meter, loss_meter,
...
def main():
    # 设置 GPU 设备
    torch.cuda.set_device(0)  # 使用第一张 GPU
