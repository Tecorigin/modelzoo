...
def fill_mesh(mesh2fill, file: str, opt):
    ...
    if os.path.exists(load_path):
        ...
        gemm_edges = np.array(mesh_data.gemm_edges)
        vs = np.array(mesh_data.vs)
        edges = np.array(mesh_data.edges)
        edges_count = mesh_data.edges_count
        ve = np.array(mesh_data.ve, dtype=object)
        v_mask = np.array(mesh_data.v_mask)
        filename = mesh_data.filename
        sides = np.array(mesh_data.sides)
        edge_lengths = np.array(mesh_data.edge_lengths)
        edge_areas = np.array(mesh_data.edge_areas)
        features = np.array(mesh_data.features)
        
        np.savez_compressed(load_path, gemm_edges=gemm_edges, vs=vs, edges=edges,
                            edges_count=edges_count, ve=ve, v_mask=v_mask,
                            filename=filename, sides=sides,
                            edge_lengths=edge_lengths, edge_areas=edge_areas,
                            features=features)
...
def build_gemm(mesh, faces, face_areas):
    ...
    for face_id, face in enumerate(faces):
        ...
        for idx, edge in enumerate(faces_edges):
            ...
            if edge not in edge2key:
                ...
                nb_count.append(0)
                edges_count += 1
            edge_areas[edge2key[edge]] += face_areas[face_id] / 3
        for idx, edge in enumerate(faces_edges):
            edge_key = edge2key[edge]
            edge_nb[edge_key][nb_count[edge_key]] = edge2key[faces_edges[(idx + 1) % 3]]
            edge_nb[edge_key][nb_count[edge_key] + 1] = edge2key[faces_edges[(idx + 2) % 3]]
    ...
    mesh.sides = np.array(sides, dtype=np.int64)
    mesh.edges_count = edges_count
    mesh.edge_areas = np.array(edge_areas, dtype=np.float32) / np.sum(face_areas)
