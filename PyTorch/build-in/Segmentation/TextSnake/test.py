...
def train(model, train_loader, criterion, scheduler, optimizer, epoch, logger):
    ...
    for i, (img, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, meta) in enumerate(train_loader):
        ...
        img, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map = img.to(device).to(memory_format=torch.channels_last), train_mask.to(device), tr_mask.to(device), tcl_mask.to(device), radius_map.to(device), sin_map.to(device), cos_map.to(device)
        ...
        print('({:d} / {:d}) - Loss: {:.4f}'.format(i, len(train_loader), loss.item()))
        ...
        # Stop training once 100 steps
        if train_step >= 500:
            print("Reached 100 steps. Stopping training.")
            return
...
def main():
    ...
    train_loader = data.DataLoader(trainset, batch_size=cfg.batch_size, shuffle=True, num_workers=cfg.num_workers, generator=torch.Generator(device='sdaa'))
    if valset:
        val_loader = data.DataLoader(valset, batch_size=cfg.batch_size, shuffle=False, num_workers=cfg.num_workers, generator=torch.Generator(device='sdaa'))
    else:
        valset = None
