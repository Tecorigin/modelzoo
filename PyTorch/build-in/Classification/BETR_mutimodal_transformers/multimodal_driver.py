...
def train_epoch(model: nn.Module, train_dataloader: DataLoader, optimizer, scheduler):
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        #     f.write(f"{epoch_i},{step},{loss.item():.6f}\n")
    ...
    return tr_loss / nb_tr_steps, step_losses
...
def train(
    ...
    log_file = f"training_log_{args.dataset}_{args.model}.txt"
    step_log_file = f"step_losses_{args.dataset}_{args.model}.txt"
    
    with open(log_file, 'w') as f:
        f.write("Epoch,Train Loss,Valid Loss,Test Acc,Test MAE,Test Corr,Test F1 Score\n")
    
    with open(step_log_file, 'w') as f:
        f.write("Epoch,Step,Loss\n")

    for epoch_i in range(int(args.n_epochs)):
        train_loss_result = train_epoch(model, train_dataloader, optimizer, scheduler)
        
        if isinstance(train_loss_result, tuple):
            train_loss, step_losses = train_loss_result
        else:
        ...
        with open(log_file, 'a') as f:
            f.write(f"{epoch_i},{train_loss:.6f},{valid_loss:.6f},{test_acc:.6f},{test_mae:.6f},{test_corr:.6f},{test_f_score:.6f}\n")
        
        if step_losses:
            with open(step_log_file, 'a') as f:
                for step, loss in enumerate(step_losses):
                    f.write(f"{epoch_i},{step},{loss:.6f}\n")
