...
def train_one_epoch(args, epoch, model, criterion, optimizer, train_loader, device, scaler, scheduler=None, train_sampler=None, global_step_start=0, max_global_steps=None):
    ...
    for i, (images, targets) in enumerate(train_loader):
        ...
        # 输出格式调整为：Epoch[23]:step[1/32] step_train_loss 3.0075 acc1 25.95 acc5 54.46
        # 使用 i+1 / iters 更贴近人类可读的“第几步 / 总步数（该 epoch 内）”
        if ((global_step % args.print_freq == 0) or (i == iters - 1)) and ((dist.get_rank() if dist.is_initialized() else 0) == 0):
            lr = optimizer.param_groups[0]['lr']
            # note: losses.val is 当前 batch 的 loss（经过 grad_accum 处理后还原），losses.avg 是到目前为止的 epoch 平均
...
def main():
    ...
    while True:
        ...
        # 如果启用了按 steps 的模式且已经达到上限，直接退出 main（跳过 validate）
        if max_global_steps is not None and global_step >= max_global_steps:
            if rank == 0:
                print(f"[Main] 达到 max_global_steps={max_global_steps}（global_step={global_step}），提前退出训练（跳过验证）。")
            # 直接返回 main()，不再执行后续 validate / 保存逻辑
            return
