...
def Model(num_classes=1000, model_type="vovnet39", in_ch=3, dropout=0.2):
    """
    简化工厂函数，用于快速实例化 VoVNet 模型
    Args:
        num_classes (int): 分类数
        model_type (str): vovnet19, vovnet27_slim, vovnet39, vovnet57, vovnet99
        in_ch (int): 输入通道数，默认 3 (RGB)
        dropout (float): 分类器 dropout，默认 0.2
    Returns:
        VoVNet 实例
    """
    return VoVNet(
