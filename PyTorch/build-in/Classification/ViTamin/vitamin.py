...
class GeGluMlp(nn.Module):
    def __init__(
            ...
            act_layer = None,
            drop = 0.0,
            **kwargs,            # <-- accept and ignore extra timm kwargs (norm_layer, etc.)
    ):
        super().__init__()
...
class HybridEmbed(nn.Module):
    ...
    def __init__(
            ...
            **kwargs,            # <-- accept extra kwargs (e.g. device) from timm
    ):
        super().__init__()
...
# welo: minimal CIFAR-friendly factory — never loads pretrained weights, no extra prints
def Model(num_classes = 100):
    model = timm.create_model('vitamin_small', pretrained=False, num_classes=num_classes, img_size=224)
    return model