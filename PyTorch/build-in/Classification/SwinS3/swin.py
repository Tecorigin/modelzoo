    ...
    def __init__(self, dim, out_dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads,    # 修改：去掉 [i]，直接使用传入的 int
                                 window_size=window_size, # 修改：去掉 [i]
                                 shift_size=0 if (i % 2 == 0) else window_size // 2, # 修改：去掉 [i]
                                 mlp_ratio=mlp_ratio,     # 修改：去掉 [i]
                                 qkv_bias=qkv_bias, qk_scale=qk_scale,
                                 drop=drop, attn_drop=attn_drop,
                                 # drop_path 是一个列表，所以这里保留逻辑
...
def Model(num_classes=100):
    """
    创建一个 SSSTransformer 模型实例 (默认配置对应 Swin-Tiny)
    用于 ImageNet 训练。
    
    Args:
        num_classes (int): 分类数量，默认 100 (ImageNet-100) 或 1000.
    """
    model = SSSTransformer(
        img_size=224,
        patch_size=4,
        in_chans=3,
        ...
        drop_rate=0.0,
        attn_drop_rate=0.0,
        drop_path_rate=0.2, # Swin-Tiny 在 ImageNet 上通常使用 0.2 的 drop_path
        ape=False,
        patch_norm=True,
...
if __name__ == "__main__":
    ...
    # 创建一个伪造的输入张量 (Batch_Size, Channels, Height, Width)
    dummy_input = torch.randn(1, 3, 224, 224)
    
    # 前向传播
    output = net(dummy_input)
    print(f"Input shape: {dummy_input.shape}")
    print(f"Output shape: {output.shape}") # 应该是 [1, 1000]
    
    # 计算 FLOPs (需要调用类中定义的 flops 方法)
    try:
        print(f"Model FLOPs: {net.flops() / 1e9:.2f} G")
