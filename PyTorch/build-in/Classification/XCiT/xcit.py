...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, B, H, W):
        ...
        device = not_mask.device  # 保存原始设备

        # 在 CPU 上计算累积和，保证 deterministic
        y_embed = not_mask.to('cpu').cumsum(1, dtype=torch.float32).to(device)
        x_embed = not_mask.to('cpu').cumsum(2, dtype=torch.float32).to(device)
...
def conv3x3(in_planes, out_planes, stride=1):
    ...
    return torch.nn.Sequential(
        ...
        nn.BatchNorm2d(out_planes)  # ← 改成这个！
    )
...
class ConvPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        if patch_size[0] == 16:
            ...
            # For CIFAR-32: produce 8x8 patches from 32x32 input using two stride-2 stages:
            # 32 -> 16 -> 8 spatially. Use a compact two-stage conv stack.
            self.proj = torch.nn.Sequential(
                conv3x3(3, embed_dim // 4, 2),   # 32 -> 16
                nn.GELU(),
                conv3x3(embed_dim // 4, embed_dim, 2),  # 16 -> 8, output embed_dim channels
            )
        else:
...
class LPI(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = self.bn(x)
        x = self.conv2(x)
        x = x.reshape(B, C, N).contiguous().permute(0, 2, 1)
...
class ClassAttentionBlock(nn.Module):
    ...
    def forward(self, x, H, W, mask=None):
        ...
        if self.tokens_norm:
            ...
            # x[:, 0:1] = self.norm2(x[:, 0:1])  #welo
            cls_norm = self.norm2(x[:, 0:1])
            x = torch.cat([cls_norm, x[:, 1:]], dim=1).contiguous()
...
# welo add
# --- helper to ensure divisible dims (add this near the top of xcit.py, after imports) ---
def ensure_divisible(value, divisor):
    """Return the smallest integer >= value that is divisible by divisor."""
    if divisor <= 0:
        raise ValueError("divisor must be > 0")
    return int((int(value) + divisor - 1) // divisor) * divisor

# --- 注册用于 CIFAR-32 的 XCiT 变体 ---
@register_model
def xcit_cifar32_patch4(pretrained=False, num_classes=10, *,  # 默认 CIFAR-10 的 num_classes=10，可传 100 用于 CIFAR-100
                        ...
                        cls_attn_layers=1, use_pos=True, eta=1e-5, tokens_norm=False,
                        **kwargs):
    """
    XCiT variant for CIFAR-32.
    - Default: patch_size=4 -> (32/4)^2 = 8x8 = 64 patches
    - Lightweight: smaller embed_dim and depth suitable for CIFAR datasets.
    - If you want CIFAR-100, call with num_classes=100.
    """
    # basic sanity checks and adjustments
    if img_size % patch_size != 0:
        raise ValueError(f"img_size ({img_size}) must be divisible by patch_size ({patch_size}).")
    if patch_size not in (4, 8, 16):
        raise ValueError("xcit_cifar32_patch4 supports patch_size 4, 8 or 16 (for CIFAR/ImageNet use).")

    # ensure embed_dim divisible by num_heads
    embed_dim = ensure_divisible(embed_dim, num_heads)

    # Create model with lightweight defaults
    model = XCiT(
        img_size=img_size,
        patch_size=patch_size,
        in_chans=3,
        ...
        patch_proj='conv',  # informational
        **kwargs
    )

    # Use the (updated) ConvPatchEmbed implementation (supports patch_size 4/8/16)
    model.patch_embed = ConvPatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=3, embed_dim=embed_dim)

    # default cfg suitable for CIFAR datasets
    model.default_cfg = _cfg(
        url='',
    ...
    if pretrained:
        raise NotImplementedError("Pretrained weights not available for CIFAR XCiT variant")

    return model


# convenience factory expected by your external code: Model(100) -> CIFAR-100 XCiT
def Model(num_classes):
    r"""Return XCiT model customized for CIFAR. Usage: Model(100) for CIFAR-100."""
    return xcit_cifar32_patch4(pretrained=False, num_classes=num_classes)