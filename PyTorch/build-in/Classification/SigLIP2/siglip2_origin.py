...
# 假设 configuration_siglip2 已经存在，或者你需要根据实际情况定义这两个配置类
# 如果没有配置文件，下面提供了一个最小化的 Mock 类以便代码运行
try:
...
class Siglip2ImageNetWrapper(nn.Module):
    """
    SigLIP 2 的包装器，用于适配标准的 ImageNet 训练循环。
    主要作用是自动根据输入生成 `spatial_shapes` 参数。
    """
    ...
    def forward(self, x, labels=None):
        ...
        Args:
            ...
            labels: [Batch] (可选)
        """
        # 1. 自动计算 spatial_shapes
        # SigLIP 2 需要知道每张图的 (Height, Width) 用于插值位置编码
        # 在 ImageNet 训练中，一个 Batch 内的图片通常大小一致
        b, c, h, w = x.shape
        device = x.device
        
        # 构造形状为 (Batch, 2) 的 tensor，存储 [h, w]
        spatial_shapes = torch.tensor([[h, w]], device=device).expand(b, 2)
        
        # 2. 调用原始模型
        # 注意：这里假设输入的 x 就是 pixel_values
        if labels is not None:
            return self.model(pixel_values=x, spatial_shapes=spatial_shapes, labels=labels)
        else:
            return self.model(pixel_values=x, spatial_shapes=spatial_shapes)

def Model(num_classes=1000, model_size='base', image_size=224, patch_size=16, use_wrapper=True):
    """
    工厂函数：构建用于 ImageNet 训练的 SigLIP 2 模型。
    
    Args:
        num_classes (int): 分类数量 (ImageNet 默认为 1000)
        model_size (str): 'base', 'large', 'so400m'
        image_size (int): 输入图像分辨率 (用于计算 num_patches)
        patch_size (int): Patch 大小
        use_wrapper (bool): 是否包裹一层 Wrapper 以自动处理 spatial_shapes。
                            如果你是直接替换 ResNet/ViT，建议设为 True。
    
    Returns:
        model: 配置好的 PyTorch 模型
    """
    
    # 1. 定义不同规模的超参数
    # 参考 SigLIP 2 论文或常见 ViT 设置
    configs = {
        'base': {
            'hidden_size': 768,
    ...
    param = configs[model_size]
    
    # 2. 计算 num_patches (SigLIP2VisionEmbeddings 需要这个参数)
    # 注意：虽然 SigLIP 2 支持动态分辨率，但在初始化位置编码权重时，
    # 依然需要一个基准的 max_num_patches 或者 base_resolution
    num_patches = (image_size // patch_size) ** 2

    # 3. 实例化 Vision Config
    vision_config = Siglip2VisionConfig(
        hidden_size=param['hidden_size'],
        intermediate_size=param['intermediate_size'],
        num_hidden_layers=param['num_hidden_layers'],
        num_attention_heads=param['num_attention_heads'],
        num_channels=3,
        image_size=image_size,
        patch_size=patch_size,
        num_patches=num_patches, # 关键参数
        layer_norm_eps=1e-6,
        attention_dropout=0.0,
        hidden_act="gelu", # 或者 "gelu_pytorch_tanh"
        vision_use_head=False, # 分类任务通常不需要原始的 PoolingHead，而是用 GAP
        _attn_implementation="sdpa" # 推荐使用 torch 的 SDPA 加速
    )
    ...
    config = Siglip2Config(
        ...
        problem_type="single_label_classification" if num_classes > 1 else "regression"
    )
    
    # 5. 创建模型
    # Siglip2ForImageClassification 已经在你提供的代码中定义了
    model = Siglip2ForImageClassification(config)
    
    # 6. (可选) 包裹 Wrapper 以自动处理 spatial_shapes
    if use_wrapper:
        model = Siglip2ImageNetWrapper(model)
