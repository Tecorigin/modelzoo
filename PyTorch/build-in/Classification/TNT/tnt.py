...
class Attention(nn.Module):
    def __init__(self, dim, hidden_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # self.attn_drop = nn.Dropout(attn_drop, inplace=True) # welo delete
        self.attn_drop = nn.Dropout(attn_drop) # welo add

        self.proj = nn.Linear(dim, dim)
        # self.proj_drop = nn.Dropout(proj_drop, inplace=True) # welo delete
        self.proj_drop = nn.Dropout(proj_drop) # welo add
...
@register_model
def tnt_cifar_patch4_32(pretrained=False, num_classes=10, **kwargs): #welo
    """
    Custom TNT model for CIFAR (32x32 images)
    - img_size: 32
    - patch_size: 4  --> 8x8 = 64 patches
    - inner_stride: 2 --> each patch (4x4) is split into 2x2=4 sub-patches (words)
    - Lightweight dimensions to suit small dataset
    """
    img_size = 32
    patch_size = 4
    inner_stride = 2  # so inner tokens per patch = (4/2)^2 = 4
    
    outer_dim = 192   # reduced from 384
    inner_dim = 12    # reduced from 24
    outer_num_heads = 3
    inner_num_heads = 3
    depth = 12
    ...
    model = TNT(
        ...
        drop_rate=0.1,          # add mild dropout for CIFAR
        attn_drop_rate=0.1,
        drop_path_rate=0.1,
        **kwargs
    )
    
    # Custom default config for CIFAR
    model.default_cfg = _cfg(
        url='',  # no pretrained weights by default
        input_size=(3, 32, 32),
        num_classes=num_classes,
    ...
    if pretrained:
        raise NotImplementedError("Pretrained weights not available for CIFAR-TNT")
    
    return model


# welo: your custom entry for CIFAR
def Model(num_classes):  # override or add this
    r"""Return TNT model customized for CIFAR"""
    return tnt_cifar_patch4_32(num_classes=num_classes)

# 原始代码改动：
# 1 删除所有的inplace=True 共三处。 
# 2 增加def Model(num_classes).   增加def tnt_cifar_patch4_32(pretrained=False, num_classes=10, **kwargs)   