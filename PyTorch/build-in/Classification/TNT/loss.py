...
def compare_loss(benchmark_loss_array, sdaa_loss_array):
    ...
    if mean_relative_error <= 0.05 or mean_absolute_error <= 0.0002:
        ...
        result_line = f"pass {print_str}"
        ret_bool = True
    else:
        print('fail', print_str)
        result_line = f"fail {print_str}"
        ret_bool = False

    # ========== ★ 新增部分：把上述输出内容写入 result.txt ★ ==========
    with open("loss.txt", "w") as fw:
        fw.write(f"MeanRelativeError: {mean_relative_error}\n")
        fw.write(f"MeanAbsoluteError: {mean_absolute_error}\n")
        if mean_relative_error <= mean_absolute_error:
            fw.write(f"Rule,mean_relative_error {mean_relative_error}\n")
        else:
            fw.write(f"Rule,mean_absolute_error {mean_absolute_error}\n")
        fw.write(result_line + "\n")

    # =========================================================

    return ret_bool, print_str 

def parse_string(string):
    pattern=r"rank : (-?\d+)  train.loss : ([\d\.e-]+)"
    pattern1=r"loss ([\d\.e-]+)"
    pattern2 =r"train_loss ([\d\.e-]+)"
    match = re.findall(pattern, string) or re.findall(pattern1, string) or re.findall(pattern2, string)
    #print("xxxxx",match)
    return match
