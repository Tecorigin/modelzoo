...
# 问题由底层 tecodnn（torch-sdaa 后端）不支持 groups>1 与 dilation>1 同时出现的卷积配置，当模型在 SKConv 某些分支构造 kernel=3, dilation=2, padding=2, groups=32 时，底层返回 TECODNN_STATUS_NOT_SUPPORTED 并抛出错误，导致训练中断。
