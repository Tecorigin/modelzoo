...
class DebugConv2d(nn.Conv2d):
    def forward(self, x):
        try:
            out = super().forward(x)
            print(f"[Debug][OK] Conv2d: "
            ...
            print(f"[Debug][ERROR] Conv2d failed: "
                  f"in={tuple(x.shape)}, kernel={self.kernel_size}, "
                  f"stride={self.stride}, padding={self.padding}, "
                  f"dilation={self.dilation}, groups={self.groups}")
            raise e

# [Debug][ERROR] Conv2d failed: in=(128, 128, 56, 56), kernel=(3, 3), stride=(1, 1), padding=(2, 2), dilation=(2, 2), groups=32

# 底层返回 TECODNN_STATUS_NOT_SUPPORTED 并抛出错误，导致训练中断。
    ...
    def __init__(self, channels, branches=2, groups=32, reduce=16, stride=1, len=32):
        ...
        for i in range(branches):
            self.convs.append(nn.Sequential(
                DebugConv2d(channels, channels, kernel_size=3, stride=stride, padding=1 + i, dilation=1 + i,
        ...
        self.fc = nn.Sequential(
            DebugConv2d(channels, len, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(len),
            nn.ReLU(inplace=True)
        )
        ...
        for i in range(branches):
            self.fcs.append(
                DebugConv2d(len, channels, kernel_size=1, stride=1)
            )
...
class SKUnit(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, branches=2, group=32, reduce=16, stride=1, len=32):
        ...
        self.conv1 = nn.Sequential(
            DebugConv2d(in_channels, mid_channels, kernel_size=1, stride=1, bias=False),
        ...
        self.conv3 = nn.Sequential(
            DebugConv2d(mid_channels, out_channels, kernel_size=1, stride=1, bias=False),
        ...
        if in_channels == out_channels:  # when dim not change, input_features could be added diectly to out
            ...
            self.shortcut = nn.Sequential(
                DebugConv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(out_channels)
            )
...
class sknet(nn.Module):
    def __init__(self, num_classes, num_block_lists=[3, 4, 6, 3]):
        ...
        self.basic_conv = nn.Sequential(
            DebugConv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
        ...
        for m in self.modules():
            if isinstance(m, (DebugConv2d, nn.Linear)):
