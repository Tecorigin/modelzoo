...
def Model(num_classes):
    """
    返回一个用于训练的 Visformer 模型实例（默认 small 版，输入尺寸 224）。
    如果你想要更小的模型可改为 visformer_tiny()；或者传入额外 kwargs。
    """
    # 选择模型变体：visformer_small 或 visformer_tiny
    # 这里默认用 visformer_small；如果想要更轻量的模型，可改为 visformer_tiny
    model = volo_d1(num_classes=num_classes, return_dense=False, mix_token=False)
    return model